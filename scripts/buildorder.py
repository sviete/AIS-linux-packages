    ...
    for package_dir in directories:
        for pkgdir_name in sorted(os.listdir(package_dir)):
            ...
            if os.path.isfile(dir_path + "/build.sh"):
                new_package = TermuxPackage(
                    package_dir + "/" + pkgdir_name, fast_build_mode
                )
    ...
    for pkg in all_packages:
        for dependency_name in pkg.deps:
            if dependency_name not in pkgs_map:
                die(
                    'Package %s depends on non-existing package "%s"'
                    % (pkg.name, dependency_name)
                )
...
def main():
    ...
    pkgs_map = read_packages_from_directories(
        packages_directories, fast_build_mode, full_buildorder
    )
